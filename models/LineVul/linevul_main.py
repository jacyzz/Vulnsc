    ...
    def __init__(self, tokenizer, args, file_type="train"):
        ...
        with open(file_path) as f:
            for line in f:
                ...
                if args.enhance == "enhance":
                    funcs.append(js['func_en'])
                else:
                    funcs.append(js['func'])
        ...
        for step, batch in enumerate(bar):
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                if global_step % args.save_steps == 0:
                    ...
                    if results[args.metric]>best_f1:
                        best_f1=results[args.metric]
                        logger.info("  "+"*"*20)  
                        logger.info("  Best acc/f1:%s",round(best_f1,4))
                        logger.info("  "+"*"*20)                          
                        
                        checkpoint_prefix = 'checkpoint-best-per'
...
def test(args, model, tokenizer, test_dataset, best_threshold=0.5):
    ...
    reports = classification_report(y_trues, y_preds, target_names=['0', '1'], output_dict=True)
    ...
    prc_auc = average_precision_score(y_trues, y_preds)
    metrics = {'Acc': acc, 'Pos_pre': pos_pre, 'Pos_rec': pos_rec, 'Pos_f1': pos_f1}
    with open("../lv_result.csv","a+") as f:
        writer = csv.writer(f)
        writer.writerow([args.method, args.enhance, args.prompt, args.seed])
...
def main():
    ...
    parser.add_argument("--method", default="linevul", type=str,
                        help="method name")
    parser.add_argument("--enhance", default="enhance", type=str,
                        help="enhance or not")
    parser.add_argument("--prompt", default="basic", type=str,
                        help="prompt name")
    ...
    parser.add_argument("--metric", default=None, type=str, required=True, 
                        help="selected metrics (Acc or F1).")
    ...
    t_print = time.clock_gettime(0) - scriptStartTime
    print("Total runtime: " + str(t_print))
    with open("./lv_time.csv","a+") as f:
        writer = csv.writer(f)
        m_print = str(args.model_name_or_path.split("/")[-1])
        tr_print = "train"
        e_print = args.enhance
        s_print = "none"
        d_print = args.seed
        if '/0/' in args.train_data_file:
            s_print = "basic"
        if '/1/' in args.train_data_file:
            s_print = "behavior"
        if '/2/' in args.train_data_file:
            s_print = "oneshot"
        if '/3/' in args.train_data_file:
            s_print = "cot"

        writer.writerow([m_print, tr_print, e_print, s_print, str(d_print), str(t_print)])

    scriptStartTime = time.clock_gettime(0)
    # Evaluation
    results = {}
    if args.do_test:
        checkpoint_prefix = f'checkpoint-best-per/{args.model_name}'
        output_dir = os.path.join(args.output_dir, '{}'.format(checkpoint_prefix))  
